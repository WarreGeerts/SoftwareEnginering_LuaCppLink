...
HBITMAP Bitmap::LoadPNG(const TCHAR* pFilePath)
{
	// Convert ANSI / multibyte TCHAR* to UTF‑16 wide string
	int len = MultiByteToWideChar(CP_ACP, 0, pFilePath, -1, nullptr, 0);
	if (len <= 0)
		return nullptr;

	std::wstring widePath(len, L'\0');
	MultiByteToWideChar(CP_ACP, 0, pFilePath, -1,
		widePath.data(), len);


	Gdiplus::Bitmap* bitmapPtr = Gdiplus::Bitmap::FromFile(widePath.c_str(), false);
